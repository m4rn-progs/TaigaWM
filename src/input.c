...
void river_input_device_handle_removed(void *data, struct river_input_device_v1 *device) {
    fprintf(stdout, "INFO: Input device removed\n");

    struct RiverInputDevice *rdevice = data;
    rdevice->removed = true;
}

void river_input_device_handle_type(void *data, struct river_input_device_v1 *device, uint32_t type) {
...
void river_input_device_handle_name(void *data, struct river_input_device_v1 *device, const char *name) {
    fprintf(stdout, "INFO: Input device name: %s\n", name);
    struct RiverInputDevice *rdevice = data;
    rdevice->name = strdup(name);
}

void river_input_device_handle_done(void *data, struct river_input_device_v1 *device) {
    fprintf(stdout, "INFO: Input device done\n");
}

void river_input_manager_handle_finished(void *data, struct river_input_manager_v1 *river_input_manager) {
    fprintf(stdout, "INFO: Input manager finished\n");
}

void river_input_manager_handle_input_device(void *data, struct river_input_manager_v1 *river_input_manager, struct river_input_device_v1 *input_device) {
    ...
    if (input_config.repeat_rate > 0 && input_config.repeat_delay > 0) {
        ...
        river_input_device_v1_set_repeat_info(input_device, input_config.repeat_rate, input_config.repeat_delay);
    }

    fprintf(stdout, "INFO: Adding listener to input device.\n");
    river_input_device_v1_add_listener(input_device, &river_input_device_listener, data);
